...
res.setHeader('Access-Control-Allow-Origin', 'https://shraaghav0428.github.io');
  ...
  const prompt = `You are a helpful assistant for a word game called Word Climb.
...
Reply with ONLY the hint text, nothing else.`;
  ...
  try {
    ...
    const hint = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || 'Keep going, you\'re close!';
